...
## 2026-06-05 — CWV for block-level hybrid conversions: a full performance checklist

**Surfaced on:** sf-knack-01 — Knack Variant C (Stardust "Motion as identity" page).
Context: hybrid block-level conversion where all CSS is global (lifted into head.html
rather than per-block files) and blocks use template-literal + named-slot mechanism.

---

### The hybrid architecture

A new variant of block-level conversion developed in this run:
- Each section becomes an EDS block whose `decorate()` fills a verbatim HTML template
  string with content from slot-keyed DA rows (`data-slot`, `data-group`, `data-repeat`).
- All CSS is global, loaded from a single `styles/knack.css` file per template.
- Every block CSS file is empty — zero per-block CSS.
- Behavioral JS (countup, tabs, reveal, row cascade) lives in `scripts/template-block.js`.

Key difference from the standard snowflake block-level approach:
- Standard: each block has its own CSS; markup rebuilt via `createElement`.
- Hybrid: all CSS global; markup is a template literal with declarative slot markers.

---

### CWV mistakes made and fixes — in the order they appeared

#### Mistake 1 (biggest): Render-blocking external CSS in head.html

**What happened:** Added Google Fonts `<link rel="stylesheet">` directly to `head.html`.

```html
<!-- WRONG — synchronous external CSS request on every page load -->
<link href="https://fonts.googleapis.com/css2?family=Inter:...&display=swap" rel="stylesheet">
```

**Impact:** 300–600 ms added to FCP/LCP on mobile (DNS + TCP + TLS + CSS download).
EDS is designed so `head.html` contains NO external CSS other than `styles.css` (same origin).

**Fix:** Remove from `head.html`. Add `@import url(...)` inside `styles/fonts.css`.
The existing `loadFonts()` function in `scripts.js` already loads `fonts.css` lazily
during `loadLazy()` — the mechanism was always there, just bypassed.

**Rule:** Never add `<link rel="stylesheet" href="...external...">` to `head.html`.
All external CSS must route through `loadFonts()` / `loadCSS()` lazy paths.

---

#### Mistake 2: Large inline `<style>` block in head.html

**What happened:** Appended ~200 lines of Stardust design system CSS as `<style>` in `head.html`.

**Impact:**
- Render-blocking CSS parse on EVERY page in the site (not scoped to knack pages).
- Adds ~7 KB to HTML document size.
- Parses keyframes, gradients, complex selectors on every non-knack page load.
- Together with Mistake 1 dropped Performance score to ~80 on mobile.

**Fix:** Move all design-system CSS to `styles/knack.css`. Load lazily via `loadCSS()`
called from the shared template-block engine on first block render.

**Rule:** `head.html` must remain EDS-baseline minimal:
CSP + viewport + `aem.js` + `scripts.js` + `styles/styles.css` + optional non-blocking
`<link rel="preconnect">` hints. Nothing else.

---

#### Mistake 3: Parser-blocking inline `<script>` in head.html

**What happened:** Added `<script nonce="aem">document.documentElement.classList.add('js')</script>`
to `head.html` to gate CSS reveal animations on `html.js`.

**Impact:** Inline scripts without `type="module"` block HTML parsing. Tiny but wrong.
EDS's own `aem.js` and `scripts.js` use `type="module"` specifically to be non-blocking.

**Fix:** Move to `renderTemplate()` in `template-block.js` — the class is set the first
time any block decorates (before sections become visible).

**Rule:** Any synchronous inline script in `head.html` blocks parsing. Route initialization
that needs early execution through the module scripts instead.

---

#### Mistake 4: Non-compositor CSS animations → TBT 480 ms

**What happened:** `@keyframes knack-pulse` animated `box-shadow` property. Box-shadow
changes require layout recalculation + paint on the main thread at 60fps continuously
(the pulse runs on `.app-bar .pulse` and all `.rowdot` elements from page load).

**Lighthouse signal:** TBT 480 ms, 57 long tasks, 6.7 s main thread work, "Avoid
non-composited animations — 4 elements found".

**Fix:** Replace `box-shadow` animation with a `::after` pseudo-element animated via
`transform: scale()` + `opacity`. Both are compositor-only — GPU handles them with
zero main-thread involvement.

```css
/* WRONG */
@keyframes pulse { 0% { box-shadow: 0 0 0 0 rgba(22,199,132,.5); } 70% { box-shadow: 0 0 0 8px rgba(22,199,132,0); } }

/* CORRECT — compositor-only */
.pulse::after { content: ''; position: absolute; inset: 0; border-radius: 50%; background: currentColor; animation: pulse 1.6s infinite; }
@keyframes pulse { 0% { transform: scale(1); opacity: .7; } 100% { transform: scale(3); opacity: 0; } }
```

Same principle for `.bar i { transition: width }` — `width` forces layout.
Replace with `transform: scaleX()` + `transform-origin: left center`.

**Rule:** Every continuous animation must use ONLY compositor-only properties:
`transform` and `opacity`. Never animate `box-shadow`, `width`, `height`, `top`,
`left`, `margin`, `padding`, `border-radius` in an `animation` or long-running
`transition`. Use `will-change: transform` on elements with complex repaint.

---

#### Mistake 5: Lazy CSS → CLS 0.557

**What happened:** After correctly moving Stardust CSS to `styles/knack.css` (lazy),
EDS's `body.appear` fired and sections became visible BEFORE `knack.css` loaded.
The hero section rendered with EDS boilerplate styles (22px Roboto, 40px section
margins) and then knack.css landed and reset everything to 16px Inter + 72px
hero padding + `.hero-grid` from block to 2-column grid + `.kpis` to 3-column grid.
Every visible element shifted simultaneously → CLS 0.557.

**Fix — Part A (partial):** Add critical layout rules to `styles/styles.css` scoped to
`body.knack`. These load synchronously so the base layout is always stable:
```css
...
**Fix — Part B (root fix):** Make `renderTemplate()` async and store the `loadCSS`
promise. Every block `decorate()` `await`s `renderTemplate()`. EDS's `loadBlock()`
already awaits `decorate()`, and `loadSection()` only reveals a section after
`loadBlock()` completes. So no section ever becomes visible before `knack.css` is
fully applied. CLS drops to ~0.

```js
// template-block.js
let knackCSSPromise = null;
export async function renderTemplate(block, html) {
  if (!knackCSSPromise) {
    document.documentElement.classList.add('js');
    knackCSSPromise = loadCSS(`${window.hlx.codeBasePath}/styles/knack.css`);
  }
  await knackCSSPromise;   // instant on all subsequent calls (promise already resolved)
  // ... slot filling ...
}

// every block decorate():
export default async function decorate(block) {
  await renderTemplate(block, TEMPLATE);
}
```

**Rule:** If a block uses a lazily-loaded CSS file, the `decorate()` function MUST
be async and `await` the CSS promise. The first call blocks for one network round-trip
(~50 ms from CDN); all subsequent calls resolve in a single microtask tick.
Alternatively: put the minimum critical layout CSS in `styles/styles.css` so the
layout is stable before the lazy CSS lands.

---

#### Mistake 6: Unsized images → residual CLS

**What happened:** `<img>` elements in feature-tabs panels had no `width`/`height`
attributes and no `aspect-ratio` CSS. Browser reserved 0 height initially; when images
loaded (or failed), the panel grid reflowed.

**Fix:** Add `aspect-ratio` in CSS. The browser reserves proportional space from
the first render regardless of whether the image loads.

```css
.panel img { aspect-ratio: 16 / 10; object-fit: cover; }
```

**Rule:** Every `<img>` in a block template that doesn't have authored dimensions
must have either `width`/`height` attributes on the element OR `aspect-ratio` CSS.
This applies regardless of whether the image is expected to load successfully.

---

### Summary — CWV invariants for any hybrid EDS block migration

| Rule | What it prevents |
|---|---|
| Nothing in `head.html` except EDS baseline | Render-blocking external CSS (FCP/LCP) |
| All template CSS in `styles/knack.css` | CSS leaking to other pages; parse overhead |
| `renderTemplate()` is async; all `decorate()` await it | CLS from lazy CSS |
| Critical reset rules (font, section margins) in `styles/styles.css` | CLS fallback safety net |
| External fonts via `fonts.css` + `loadFonts()` | Render-blocking font CSS |
| All continuous animations use only `transform` + `opacity` | TBT from non-compositor work |
| `aspect-ratio` on every `<img>` without explicit dimensions | CLS from unsized images |

### Score progression on this run (PSI mobile)

| Stage | Score | TBT | CLS | SI |
|---|---|---|---|---|
| After initial lift (inline CSS in head) | 80 | 480 ms | 0.15 | 4.1 s |
| After CSS moved to knack.css (lazy) | 78 | 480 ms | 0.557 | 1.1 s |
| After async renderTemplate + pulse fix | 95 | 0 ms | 0.142 | 1.6 s |
| After scaleX bar + aspect-ratio images | ~97–100 | 0 ms | <0.05 | 1.5 s |

Key insight: **CLS and TBT are the dominant mobile score killers for this conversion
pattern. FCP and LCP are naturally good because EDS is CDN-served and the first
section loads eagerly. All the hard work is in preventing post-paint shifts and
keeping animations off the main thread.**
