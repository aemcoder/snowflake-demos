...
async function applyTemplateOverlay(main) {
  ...
  // Lift any top-level <script> resources the template declares into
  // <head>. Sources like Typekit ship font loaders as <script>, not
  // <link>. The src-only path is straightforward; for inline scripts
  // we have to rebuild the element because cloneNode skips execution.
  // Dedupe src-bearing scripts by URL; inline scripts are appended
  // verbatim (the template author owns idempotency).
