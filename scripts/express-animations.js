/* express-animations.js — extracted from source inline <script>.
 * Loaded by delayed.js (HEAD-probed). The original page loaded Lenis via a
 * CDN <script> in <head> before this inline block; here we load it first,
 * then run the body so the Lenis-init IIFE finds the global. Card / scroll
 * animations degrade gracefully to window.scrollY if Lenis fails to load.
 */
(function () {
  if (window.Lenis) { run(); return; }
  const s = document.createElement('script');
  s.src = 'https://cdn.jsdelivr.net/npm/lenis@1.3.4/dist/lenis.min.js';
  s.onload = run;
  s.onerror = run; // Lenis-init IIFE no-ops when Lenis is undefined
