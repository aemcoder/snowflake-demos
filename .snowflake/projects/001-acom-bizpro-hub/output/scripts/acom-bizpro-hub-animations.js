// Per-template animations for acom-bizpro-hub
// Loads vendored Lenis library, then runs the page-specific animation loops.
(function () {
  function start() {
        // ── Nav scroll ─────────────────────────────────────────────────────────
        ...
        function buildGrid() {
          ...
          cols.innerHTML = '';
          const n = window.innerWidth >= 768 ? 12 : 6;
          for (let i = 0; i < n; i++) {
        ...
        function toggleGrid() {
          ...
          if (gridOn) { buildGrid(); overlay.classList.add('on'); btn.classList.add('on'); }
          else { overlay.classList.remove('on'); btn.classList.remove('on'); }
        }
        window.addEventListener('resize', () => { if (gridOn) buildGrid(); }, { passive: true });
    ...
    // Expose tab switcher used by inline onclick handlers in the template.
    window.switchTab = switchTab;
  }
  const s = document.createElement("script");
  s.src = (window.hlx && window.hlx.codeBasePath ? window.hlx.codeBasePath : "") + "/scripts/acom-bizpro-hub-lenis.min.js";
  s.onload = start;
