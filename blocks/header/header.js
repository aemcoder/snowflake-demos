...
export default async function decorate(block) {
  ...
  // mobile menu
  const mnav = block.querySelector('#mnav') || document.getElementById('mnav');
  const menuOpen = block.querySelector('#menuOpen') || block.querySelector('.menu-toggle');
  const menuClose = mnav?.querySelector('#menuClose') || mnav?.querySelector('.x');
  if (mnav && menuOpen) {
    menuOpen.addEventListener('click', () => mnav.classList.add('open'));
    if (menuClose) menuClose.addEventListener('click', () => mnav.classList.remove('open'));
